...
CREATE TABLE
test_database=*# create rule orders_1_insert as on insert to orders where (price >499) do instead insert into orders_1 values (new.*);
CREATE RULE
test_database=*# create rule orders_2_insert as on insert to orders where (price <= 499) do instead insert into orders_2 values (new.*);
CREATE RULE
...
```bash
root@f1e8442f5cc3:/# pg_dump -U postgres -d test_database > test_database_dump.sql
```
